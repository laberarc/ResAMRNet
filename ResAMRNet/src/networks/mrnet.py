...
class ResAMRNet(nn.Module):
    def __init__(self, use_meta=False, row_col=True, dropout=False, do_contrast=False, force_bias=False,
        ...
        #     # self.eca
        #     )
        self.perception_net_high = nn.Sequential(ResBlock(1, self.high_dim0, stride=2,
                                      downsample=nn.Sequential(conv1x1(1, self.high_dim0, stride=2),nn.BatchNorm2d(self.high_dim0))),
                                                 ResBlock(self.high_dim0, self.high_dim, stride=2,
                                      downsample=nn.Sequential(conv1x1(self.high_dim0, self.high_dim, stride=2),nn.BatchNorm2d(self.high_dim)))
                                                 )
        self.perception_net_mid = nn.Sequential(ResBlock(self.high_dim, self.mid_dim0, stride=2,
                                      downsample=nn.Sequential(conv1x1(self.high_dim, self.mid_dim0, stride=2),nn.BatchNorm2d(self.mid_dim0))),
                                                ResBlockSp(self.mid_dim0, self.mid_dim, stride=2,
                                      downsample=nn.Sequential(conv1x1(self.mid_dim0, self.mid_dim, stride=2),nn.BatchNorm2d(self.mid_dim)))
                                                 )
        self.perception_net_low = nn.Sequential(ResBlock(self.mid_dim, self.low_dim0, stride=2,
                                      downsample=nn.Sequential(conv1x1(self.mid_dim, self.low_dim0, stride=2),nn.BatchNorm2d(self.low_dim0))),#3x3
                                                ResBlock(self.low_dim0, self.low_dim, stride=2, padding=0,
                                      downsample=nn.Sequential(conv3x3(self.low_dim0, self.low_dim, stride=2,padding=0),nn.BatchNorm2d(self.low_dim)))
                                                 )

        self.longjump = nn.Sequential(conv1x1(1, self.high_dim0, stride=2),nn.BatchNorm2d(self.high_dim0),nn.ReLU(inplace=True),
                                      conv1x1(self.high_dim0, self.high_dim, stride=2),nn.BatchNorm2d(self.high_dim),nn.ReLU(inplace=True),
                                      conv1x1(self.high_dim, self.mid_dim0, stride=2),nn.BatchNorm2d(self.mid_dim0),nn.ReLU(inplace=True),
                                      conv1x1(self.mid_dim0, self.mid_dim, stride=2),nn.BatchNorm2d(self.mid_dim))
    ...
    def reduce(self, row_features, N, K0):
    ...
    def forward(self, x):
        ...
        out,down,eca = self.perception_net_mid(input_features_high)
        longjump = self.longjump(x.view(-1, 80, 80).unsqueeze(1))
        input_features_mid = self.relu(longjump+out+down+eca)
        ...
        if self.levels[0] == '1':
            ...
            reduced_feats_high = self.reduce(row_feats_high, N, K0)
        ...
        if self.levels[1] == '1':
            ...
            reduced_feats_mid = self.reduce(row_feats_mid, N, K0)
        ...
        if self.levels[2] == '1':
            ...
            reduced_feats_low = self.reduce(row_feats_low, N, K0)
